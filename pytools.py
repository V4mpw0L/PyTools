...
def temporary_email():
    try:
        ...
        while True:
            random_email_url = "https://www.1secmail.com/api/v1/?action=genRandomMailbox&count=1"
            response = requests.get(random_email_url)
            email_address = response.json()[0]
            displayed_email_ids = set()

            email_box = Padding(
                Panel(
                    f"[bold blue]Temporary Email[/bold blue]\n"
                    f"[bold green]Your temporary email address is:[/bold green] {email_address}\n"
                    f"[cyan]Press Ctrl+C to exit and return to the main menu.[/cyan]",
                    title="[bold]Ready to Receive Emails[/bold]",
                    border_style="green",
                ),
                (0, 1),
                expand=True
            ...
            console.print(email_box)

            with Progress(
                "[bold green]Checking for emails...[/bold green]",
                transient=True,
                refresh_per_second=4,
            ) as progress:
                try:
                    while True:
                        check_email_url = f"https://www.1secmail.com/api/v1/?action=getMessages&login={email_address.split('@')[0]}&domain={email_address.split('@')[1]}"
                        response = requests.get(check_email_url)
                        response.raise_for_status()
                        emails = response.json()
                        if emails:
                            for email in emails:
                                email_id = email['id']
                                if email_id not in displayed_email_ids:
                                    email_details_url = f"https://www.1secmail.com/api/v1/?action=readMessage&login={email_address.split('@')[0]}&domain={email_address.split('@')[1]}&id={email_id}"
                                    ...
                                    table.add_column("Subject", style="dim", width=20)
                                    table.add_column("From", style="dim", width=20)
                                    table.add_column("Body", style="dim")
                                    for email in all_emails:
                                        table.add_row(email['subject'], email['from'], email['textBody'])

                                    console.print(table)
                                    displayed_email_ids.add(email_id)

                        time.sleep(5)

                except requests.exceptions.RequestException as e:
                    console.print(f"[red]Error:[/red] An error occurred while checking for emails: {e}")

    except KeyboardInterrupt:
        console.print("\n[bold]Returning to the main menu...[/bold]")
...
# Instead of using the slugify library, use a custom function to slugify the title
...
def download_video(yt, selected_stream, max_retries=3):
    try:
        ...
        video_title = slugify_title(yt.title)  # Use the custom slugify function
                ...
                with tqdm(total=total_size, unit='b', unit_scale=True, unit_divisor=1024) as bar:
                    with open(file_path, 'wb') as f:
                        for data in response.iter_content(chunk_size=1024):
                            bar.update(len(data))
                            f.write(data)
                print_box(f"Video downloaded successfully as {slugify_title(video_title)}_video.mp4 in the PlaylistVideos folder")
                return
            except requests.RequestException as e:
...
def download_audio(yt, max_retries=3):
    try:
        ...
        audio_title = slugify_title(yt.title)  # Use the custom slugify function
                ...
                with tqdm(total=total_size, unit='b', unit_scale=True, unit_divisor=1024) as bar:
                    with open(file_path, 'wb') as f:
                        for data in response.iter_content(chunk_size=1024):
                            bar.update(len(data))
                            f.write(data)
                print_box(f"Audio downloaded successfully as {slugify_title(audio_title)}_audio.mp3 in the AudiosDownloads folder")
                return
            except requests.RequestException as e:
                logging.warning(f"Network request failed. Retrying... (Error: {e})")
                retry_count += 1
...
def menu():
    while True:
        ...
        print(f"{Colors.MAGENTA}{Colors.BOLD}15.| {Colors.MAGENTA}Temporary Email{Colors.NORMAL}")
        print(f"{Colors.RED}{Colors.BOLD}16.| {Colors.RED}Exit{Colors.NORMAL}")
